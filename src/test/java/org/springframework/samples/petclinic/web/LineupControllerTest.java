...
@WebMvcTest(controllers = LineupController.class, excludeFilters = @ComponentScan.Filter(type = FilterType.ASSIGNABLE_TYPE, classes = WebSecurityConfigurer.class), excludeAutoConfiguration = SecurityConfiguration.class)
public class LineupControllerTest {
	...
	void testShowLineups() throws Exception {
		...
		mockMvc.perform(get("/leagues/{leagueId}/teams/{teamId}/lineups", TEST_LEAGUE_ID, TEST_TEAM_ID))
				.andExpect(status().isOk())
				.andExpect(model().attribute("resultados",
						Matchers.hasItem(Matchers.<Lineup>hasProperty("category", is(lineup.getCategory())))))
				.andExpect(model().attribute("resultados",
						Matchers.hasItem(Matchers.<Lineup>hasProperty("recruit1", is(lineup.getRecruit1())))))
				.andExpect(model().attribute("resultados",
						Matchers.hasItem(Matchers.<Lineup>hasProperty("recruit2", is(lineup.getRecruit2())))))
				.andExpect(model().attribute("resultados",
						Matchers.hasItem(Matchers.<Lineup>hasProperty("gp", is(lineup.getGp())))))
				.andExpect(model().attribute("resultados",
						Matchers.hasItem(Matchers.<Lineup>hasProperty("team", is(lineup.getTeam())))))
	...
	void testShowExistentLineupById() throws Exception {
		...
		mockMvc.perform(get("/leagues/{leagueId}/teams/{teamId}/lineups/{lineupId}", TEST_LEAGUE_ID, TEST_TEAM_ID,
				TEST_LINEUP_ID)).andExpect(status().isOk()).andExpect(model().attributeHasNoErrors("lineup"))
				.andExpect(model().attribute("lineup", is(lineup))).andExpect(view().name("lineups/lineupDetails"));
	}
	...
	void testShowNonExistentLineupById() throws Exception {
		...
		mockMvc.perform(get("/leagues/{leagueId}/teams/{teamId}/lineups/{lineupId}", TEST_LEAGUE_ID, TEST_TEAM_ID,
				TEST_LINEUP_ID)).andExpect(status().is3xxRedirection())
	...
	void testCreateLineupGet() throws Exception {
		...
		mockMvc.perform(get("/leagues/{leagueId}/teams/{teamId}/newLineup", TEST_LEAGUE_ID, TEST_TEAM_ID))
				.andExpect(status().isOk()).andExpect(model().attributeHasNoErrors("lineup"))
...
//				.param("league", TEST_LEAGUE_ID.toString())
				.param("team", lineup.getTeam().getId().toString()).param("gp.id", lineup.getGp().getId().toString())
...
//				.param("recruit2.pilot.nationality", lineup.getRecruit2().getPilot().getNationality().toString())
				.flashAttr("recruitsSelection", listaPilotos)).andExpect(status().is3xxRedirection())
//		.andExpect(view().name("redirect:/leagues/" + TEST_LEAGUE_ID + "/teams/" + lineup.getTeam().getId() + "/details"));
	...
	void testCrearAlineacionPostHasErrors() throws Exception {
		...
		mockMvc.perform(post("/leagues/{leagueId}/teams/{teamId}/newLineup", TEST_LEAGUE_ID, TEST_TEAM_ID).with(csrf())
				...
				// Solo falla con los param de recruit1 y 2, con los demas, da igual si son null
				// o no :(
...
//				.param("recruit2.id", lineup.getRecruit2().getId().toString()))   // Error
				.andExpect(status().isOk()).andExpect(model().attributeHasErrors("lineup"))
				.andExpect(model().attributeErrorCount("lineup", 4)).andExpect(view().name("thymeleaf/lineupsEdit"));
	}
	...
	void testEditLineupGet() throws Exception {
		...
		mockMvc.perform(get("/leagues/{leagueId}/teams/{teamId}/editLineup/{lineupId}", TEST_LEAGUE_ID, TEST_TEAM_ID,
				TEST_LINEUP_ID).with(csrf()).header("Referer", "/leagues/1/teams/1/details")
						.param("id", lineup.getId().toString()).param("category", lineup.getCategory().toString())
				...
				.andExpect(status().isOk()).andExpect(model().attribute("lineup", is(lineup)))
...
//		.andExpect(model().attribute("message", is("Lineup not found!")))
	...
	void testEditLineupPost() throws Exception {
		...
		mockMvc.perform(post("/leagues/{leagueId}/teams/{teamId}/editLineup/{lineupId}", TEST_LEAGUE_ID, TEST_TEAM_ID,
				TEST_LINEUP_ID).with(csrf())
...
//				.param("recruit2.id", lineup.getRecruit2().getId().toString()))   // Error
	...
	void testDeleteLineup() throws Exception {
		...
		mockMvc.perform(get("/leagues/{leagueId}/teams/{teamId}/delete/{lineupId}", TEST_LEAGUE_ID, TEST_TEAM_ID,
				TEST_LINEUP_ID).header("Referer", "/leagues/1/teams/1/details")).andExpect(status().is3xxRedirection())
